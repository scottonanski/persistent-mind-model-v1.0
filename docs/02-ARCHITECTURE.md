...
- Concept Token Layer (CTL): a concept graph built purely from ledger events over semantic tokens (e.g., identity, policy, governance, topic concepts). CTL has **no built‑in ontology or runtime seeding**; instead, concepts arise when events (often driven by `prompts.py`) carry concept tokens that are recorded as `concept_define`, `concept_bind_event`, or `concept_relate` events. CTL is maintained incrementally in the runtime loop and is fully rebuildable on demand for context rendering. It is used to track high‑level notions like system maturity, autonomy behavior, and governance threads.
