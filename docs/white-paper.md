...
- **Concept Token Layer (CTL)** (`pmm/core/concept_graph.py`) maintains a concept graph over semantic tokens (identity, policy, governance, topic, ontology) that appear in the ledger. There is **no built‑in ontology or runtime seeding**; concepts exist only if events (often prompted by `prompts.py`) introduce tokens that are recorded as `concept_define`, `concept_bind_event`, or `concept_relate` events. CTL binds key system events (e.g., `stability_metrics`, `coherence_check`, `policy_update`, `summary_update`, autonomy reflections) to those tokens and is fully reconstructable from the ledger.
