...
def test_replay_narration_marks_unverified_inter_ledger_refs(tmp_path):
    ...
    log.append(
        ...
        content="REF: ../other_pmm_v2.db#47",
        meta={"verified": False},
    )
    log.append(
        kind="inter_ledger_ref",
    ...
    assert (
        "(unverified ref: REF: ../other_pmm_v2.db#47 - create dummy for test)" in text
    )
    # Should appear twice
    assert (
        text.count(
            "(unverified ref: REF: ../other_pmm_v2.db#47 - create dummy for test)"
        )
        == 2
    )
