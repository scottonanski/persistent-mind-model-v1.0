...
def test_validate_claims_basic():
    ...
    ok, msg = validate_claim(Claim(type="event_existence", data={"id": e1}), log, mirror)
    assert ok and msg == "event exists"

    ok, msg = validate_claim(Claim(type="event_existence", data={"id": 99999}), log, mirror)
    ...
    ok, msg = validate_claim(Claim(type="commitment_status", data={"cid": "abcd", "open": True}), log, mirror)
