...
def sql_query(query):
    ...
    try:
        response = requests.post(
            f"{API_BASE}/events/sql", json={"query": query}, timeout=10
        )
...
def verify_architectural_honesty():
    ...
    return {"validator_warnings": warnings, "self_corrections": corrections}
...
def verify_stage_progression():
    ...
    return {"stage_updates": stages, "metrics_progression": metrics}
...
def generate_report(results):
    ...
    with open(filename, "w") as f:
        f.write("# Echo Session Verification Report\n")
        ...
        f.write(
            f"- **Open Commitments**: {results.get('commitment_system', {}).get('commitment_count', 0)}\n\n"
        )
...
def main():
    ...
    try:
        ...
        warnings = len(
            results["architectural_honesty"]["validator_warnings"].get("rows", [])
        )
