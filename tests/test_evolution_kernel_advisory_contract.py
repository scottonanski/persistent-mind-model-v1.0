...
class SpyEventLog:
    ...
    def read_tail(self, limit: int = 1000):
        return self.events[-limit:]

    def read_after_id(self, after_id: int | None = None, limit: int = 10000):
        return []
...
class NoopTracker:
    def _open_map_all(self, _events):
        return {}


class AlwaysOKCooldown:
    def should_reflect(self, now: float, novelty: float):
        return True, "test"
...
def test_trigger_reflection_may_append_only_reflection():
    ...
    def _fake_eval(**_):
        return {
            ...
            "trait_adjustments": {"conscientiousness": {"target": 0.6}},
            "ias": 0.2,
            "gas": 0.2,
            "open_commitments": [],
        }
